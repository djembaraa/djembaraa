# Hi 👋, I'm Djembar Arafat
### Full-Stack Developer & UI/UX Designer

<p align="left">
  <img src="https://komarev.com/ghpvc/?username=djembaraa&label=Profile%20views&color=0e75b6&style=flat-square" alt="Profile Views" />
</p>

---

## 👨‍💻 About Me

Hi! I am a **Full-Stack Web Developer** and **UI/UX Designer**. I specialize in building fast, secure, and intuitive web applications with clean, pixel-perfect visual interfaces.

- 🔭 **Currently Working On:** E-Commerce Platform (**Gulma**) & Real-Time Web Applications
- 🌱 **Currently Learning:** AI Integration, Web3 Development, Data Analytics, & Web Application Security
- 💼 **Portfolio:** [djembara.com](https://www.djembara.com/)
- 📝 **Blog & Articles:** [djembara.com/blog](https://www.djembara.com/blog)
- ⚡ **Philosophy:** *"Bridging clean, structured code architecture with pixel-perfect interface design."*

---

## 🛠️ Tech Stack & Skills

<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=ts,js,html,css,react,next,tailwind,redux,nodejs,express,postgres,mysql,mongodb,figma,git,docker,postman,linux" alt="Tech Stack" />
  </a>
</p>

---

## 🚀 Featured Projects

- 🛒 **[Gulma](https://www.djembara.com)** — Modern E-Commerce platform built with **Next.js, TypeScript, Tailwind CSS, & PostgreSQL**.
- 💬 **ChatYo** — Real-Time messaging app powered by **Pusher WebSocket, Node.js, & JWT Authentication**.
- 💳 **SLPPOS** — Web-based Point of Sale (POS) system for inventory management and sales transactions.

---

## 🎮 Contribution Activity

<p align="center">
  <img src="https://raw.githubusercontent.com/platane/snk/output/github-contribution-grid-snake.svg" alt="Snake Contribution Animation" width="100%" />
</p>

---

## 📬 Connect With Me

<p align="left">
  <a href="mailto:djembararafat98@gmail.com"><img src="https://img.shields.io/badge/Email-djembararafat98%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
  <a href="https://linkedin.com/in/djembar-arafat-9a6602178"><img src="https://img.shields.io/badge/LinkedIn-Djembar_Arafat-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="https://www.djembara.com"><img src="https://img.shields.io/badge/Portfolio-djembara.com-000000?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" /></a>
  <a href="https://dribbble.com/Djembara"><img src="https://img.shields.io/badge/Dribbble-Djembara-EA4C89?style=for-the-badge&logo=dribbble&logoColor=white" alt="Dribbble" /></a>
  <a href="https://github.com/djembaraa"><img src="https://img.shields.io/badge/GitHub-djembaraa-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>
</p>
